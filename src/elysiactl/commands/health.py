...
from ..utils.display import console, create_health_panel, print_section_header
...
def health_command(
    ...
    if cluster:
        ...
        asyncio.run(_perform_cluster_verification(weaviate, collection, quick, json_output))
        return
...
async def _perform_cluster_verification(
    weaviate_service: WeaviateService, collection: str | None, quick: bool, json_output: bool
) -> None:
    ...
    if json_output:
        ...
        output_data = {
            ...
            "cluster": {
                ...
                "nodes": {"expected": result.expected_nodes, "actual": result.node_count},
            },
            "collections": {
                "system": {
                    name: {
            ...
            "warnings": [
                {"message": warning.message, "node": warning.node} for warning in result.warnings
            ],
            "error": result.error,
        }
    ...
    if result.system_collections:
        ...
        for name, status in result.system_collections.items():
            if status.exists:
                ...
                if (
                    status.replication_factor == result.node_count
                    and nodes_with_data == result.node_count
                ):
                    replication_text += " ✓"
                    repl_style = "green"
    ...
    if result.derived_collections:
        ...
        for name, status in result.derived_collections.items():
            ...
            derived_table.add_row(name, Text(status_icon, style=status_style), replication_text)
    ...
    if result.issues or result.warnings:
        ...
        summary_panel = Panel(
            "\n".join(
                [f"• {issue.message}" for issue in result.issues[:5]]
