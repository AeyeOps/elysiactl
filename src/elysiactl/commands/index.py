...
async def ensure_collection_schema(collection_name: str | None = None) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if factor != expected_factor:
                    console.print(
                        f"[yellow]⚠ Collection {collection_name} exists with replication factor={factor}, expected={expected_factor}[/yellow]"
                    )
                    console.print(
                        "[yellow]  Consider using 'elysiactl repair config-replication' to fix[/yellow]"
                    )
            ...
            schema = {
                ...
                "properties": [
                    ...
                    {"name": "line_count", "dataType": ["int"], "description": "Number of lines"},
                    {
                        "name": "last_modified",
                        "dataType": ["date"],
            ...
            create_response = await client.post(
                f"{config.services.weaviate_base_url}/schema", json=schema
            )

            if create_response.status_code in [200, 201]:
                console.print(
                    f"[green]✓[/green] Created collection {collection_name} with replication factor={config.collections.replication_factor}"
                )
                return True
            else:
...
async def index_file(file_path: Path, repo_name: str, repo_root: Path) -> dict | None:
...
async def index_repository(
    repo_path: Path, collection_name: str, progress: Progress, task_id
) -> tuple[int, int]:
    ...
    async with httpx.AsyncClient(timeout=config.processing.long_timeout) as client:
        try:
            response = await client.post(
                ...
                json={
                    "objects": [{"class": collection_name, "properties": obj} for obj in objects]
                },
            )
...
def enterprise(
    ...
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Show what would be indexed without doing it"
    ),
    collection: str | None = typer.Option(None, "--collection", help="Collection name to use"),
):
    ...
    all_repos = sorted(
        [
            d
            for d in enterprise_dir.iterdir()
            if d.is_dir()
...
def sync(
    stdin: Annotated[
        bool, typer.Option("--stdin", help="Read file paths from standard input")
    ] = False,
    dry_run: Annotated[
        bool,
        typer.Option("--dry-run", help="Show what would be changed without modifying Weaviate"),
    ] = False,
    collection: Annotated[
        str | None, typer.Option("--collection", help="Target collection name")
    ] = None,
    verbose: Annotated[
        bool, typer.Option("--verbose", "-v", help="Show detailed progress for each file")
    ] = False,
    # Performance options
    ...
    batch_size: Annotated[
        int, typer.Option("--batch-size", help="Batch size for processing")
    ] = None,
    no_optimize: Annotated[
        bool, typer.Option("--no-optimize", help="Disable performance optimizations")
    ] = False,
):
    ...
    asyncio.run(
        sync_changes_async(
            stdin, collection, dry_run, verbose, parallel, workers, batch_size, no_optimize
        )
    )


async def sync_changes_async(
    use_stdin: bool,
...
def errors(
    ...
    summary: Annotated[
        bool, typer.Option("--summary", help="Show error summary statistics")
    ] = False,
    ...
    if summary:
        ...
        if summary_data["error_counts"]:
            ...
            sorted_errors = sorted(
                summary_data["error_counts"].items(), key=lambda x: x[1], reverse=True
            )
        ...
        if cb_states:
            ...
            for operation, status in cb_states.items():
                ...
                console.print(
                    f"  {operation}: [{state_color}]{status['state']}[/{state_color}] ({status['failures']} failures)"
                )
...
@app.command()
def status(
    run_id: str | None = typer.Option(None, "--run-id", help="Show status for specific run"),
    ...
    collection: str | None = typer.Option(
        None, "--collection", help="Collection name to check (legacy)"
    ),
    ...
    if failed:
        ...
        if failed_lines:
            ...
            for failed_line in failed_lines[:10]:  # Show first 10
                console.print(
                    f"  Line {failed_line['line_number']}: {failed_line['file_path']} - {failed_line['error_message']}"
                )
            if len(failed_lines) > 10:
                console.print(f"  ... and {len(failed_lines) - 10} more")


async def check_collection_status_async(collection_name: str | None, json_output: bool):
...
def analyze(
    ...
    if summary:
        ...
        table.add_row(
            "Tier 1 (Plain)", str(stats["tier_1_plain"]), "mgit would embed as plain text"
        )
        table.add_row("Tier 2 (Base64)", str(stats["tier_2_base64"]), "mgit would embed as base64")
        table.add_row(
            "Tier 3 (Reference)", str(stats["tier_3_reference"]), "mgit would use file references"
        )
        table.add_row(
            "Skipped (Binary)", str(stats["skipped_binary"]), "mgit would skip binary files"
        )
        table.add_row(
            "Skipped (Vendor)", str(stats["skipped_vendor"]), "mgit would skip vendor dirs"
        )
        ...
        if total_files > 0:
            indexed_files = (
                stats["tier_1_plain"] + stats["tier_2_base64"] + stats["tier_3_reference"]
            )
            ...
            console.print(
                f"  Would be indexed: {indexed_files} ({indexed_files / total_files * 100:.1f}%)"
            )
            console.print(
                f"  Content embedded: {embedded_files} ({embedded_files / total_files * 100:.1f}%)"
            )
    ...
    if verbose:
        ...
        for file_path in paths:
            ...
            tier_name = {0: "Error", 1: "Plain", 2: "Base64", 3: "Ref"}.get(
                analysis.predicted_tier, "Unknown"
            )

            notes = (
                analysis.skip_reason if analysis.is_skippable else f"Binary: {analysis.is_binary}"
            )
    ...
    try:
        ...
        if show_stats:
            ...
            if total_changes > 0:
                ...
                console.print(
                    f"  Content embedded: {embedded_count} ({embedded_count / total_changes * 100:.1f}%)"
                )
...
def perf(
    show_config: Annotated[
        bool, typer.Option("--config", help="Show performance configuration")
    ] = False,
    benchmark: Annotated[
        bool, typer.Option("--benchmark", help="Run performance benchmark")
    ] = False,
    workers: Annotated[int, typer.Option("--workers", help="Number of parallel workers")] = 8,
    batch_size: Annotated[
        int, typer.Option("--batch-size", help="Batch size for processing")
...
def tune(
    target_files: Annotated[
        int, typer.Option("--target-files", help="Target number of files to optimize for")
    ] = 10000,
    target_time: Annotated[
        int, typer.Option("--target-time", help="Target completion time in seconds")
    ] = 300,
):
