...
class ElysiaService:
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "port": self.port,
            "health": "healthy" if self._check_health() else "unhealthy" if is_running else "unknown"
        }
    ...
    def _get_process_stats(self) -> Dict[str, Any]:
        ...
        if pid and is_process_running(pid):
            try:
                ...
                stats["create_time"] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(process.create_time()))
                ...
                stats["error"] = f"Cannot access process info: {str(e)}"
        else:
    ...
    def _get_recent_errors(self, count: int) -> List[str]:
        ...
        try:
            ...
            try:
                result = run_command(
                    ["journalctl", "-u", "elysia*", "--no-pager", "-n", str(count * 2), "--since", "1 hour ago"],
                    timeout=5
                )
                if result.returncode == 0:
                    ...
                    for line in lines:
                        ...
                        if any(keyword in line_lower for keyword in ["error", "exception", "fail", "traceback", "critical"]):
                ...
                for log_path in log_locations:
                    if os.path.exists(log_path):
                        try:
                            ...
                            errors.append(f"Error reading {log_path}: {str(e)}")
            ...
            errors.append(f"Error retrieving logs: {str(e)}")
