...
class ContentResolver:
    ...
    def __init__(self):
        ...
        if self.use_mime_detection:
            try:
                ...
                console.print(
                    "[dim]libmagic not available, using file extension-based MIME detection[/dim]"
                )
            except Exception:
    ...
    def resolve_content_from_reference(self, content_ref: str) -> str | None:
        """Resolve content from file reference (Tier 3 fallback)."""
        try:
            if not content_ref.startswith("/"):
                console.print(
                    f"[yellow]Invalid content reference (must be absolute path): {content_ref}[/yellow]"
                )
                return None

            with open(content_ref, encoding="utf-8", errors="ignore") as f:
                return f.read()
    ...
    def decode_base64_content(self, base64_content: str) -> str | None:
    ...
    def create_optimized_change(
        self, file_path: str, operation: str, line_number: int
    ) -> dict[str, Any]:
