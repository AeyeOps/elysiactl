...
class WeaviateService:
            ...
            nodes.append(
                {
                    ...
                    "port": config["port"],
                    "health": "healthy"
                    if node_healthy["status"] == "healthy"
                    else "unhealthy"
                    if pid
                    else "unknown",
                }
    ...
    def _check_node(self, port: int) -> dict[str, Any]:
        ...
        node_health = {"port": port, "status": "unknown", "response_time": None, "error": None}
    ...
    def _check_collection_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            with httpx.Client(timeout=5.0) as client:
                ...
                hostname = config.services.weaviate_hostname
                response = client.get(
                    f"{config.services.weaviate_scheme}://{hostname}:{self.port}/v1/schema/ELYSIA_CONFIG__"
                )
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        collection_status["replication_factor"] = schema.get(
                            "replicationConfig", {}
                        ).get("factor", 1)
                    except:
                        pass
                ...
                for port in ports:
                    try:
                        ...
                        if node_response.status_code == 200:
                            ...
                            elysia_collections = [
                                c for c in classes if c.get("class") == "ELYSIA_CONFIG__"
                            ]
    ...
    def _get_container_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            result = run_command(["docker-compose", "ps", "--format", "json"], cwd=self.work_dir)
    ...
    def _get_recent_errors(self, count: int) -> list[str]:
        ...
        try:
            ...
            logs.append(f"Error retrieving logs: {e!s}")
        ...
        try:
            ...
            weaviate_object = {
                ...
                "properties": {
                    ...
                    "last_indexed": datetime.now(UTC).isoformat() + "Z",
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    f"{self.base_url}/objects", json=weaviate_object, timeout=30.0
                )
    ...
    async def batch_insert_objects(self, collection: str, objects: list[dict[str, Any]]) -> bool:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(f"{self.base_url}/batch/objects", json=batch_request)
                return response.status_code in [200, 201]
        except Exception:
            return False
