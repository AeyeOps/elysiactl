...
from .error_handling import ErrorContext, get_error_handler, get_error_handler_with_config
...
class SQLiteCheckpointManager:
    ...
    def start_run(
        self, collection_name: str, dry_run: bool = False, input_source: str = "stdin"
    ) -> str:
        """Start a new sync run and return run_id."""
        run_id = f"sync_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}_{os.getpid()}_{uuid.uuid4().hex[:8]}"
...
def parse_input_line(line: str, line_number: int) -> dict | None:
    ...
    try:
        ...
        if isinstance(data, str):
            ...
            return {"line": line_number, "path": data, "op": "modify", "repo": None}
...
async def _build_weaviate_object(change: dict, collection: str) -> dict | None:
...
def _resolve_content(change: dict) -> str | None:
...
async def process_single_change(
    ...
    try:
        return await error_handler.execute_with_retry(
            _process_change_inner, context, change, weaviate, embedding, collection, dry_run
        )
    except Exception as e:
...
async def _process_change_inner(
    ...
    if operation == "delete":
        ...
        if dry_run:
            console.print(
                f"[blue]Would {operation.upper()}: {file_path} ({len(content)} chars)[/blue]"
            )
            return True
...
async def process_index_batch(
    ...
    for change in changes:
        ...
        try:
            ...
            results.append(
                {"success": False, "operation": operation, "path": file_path, "error": str(e)}
            )
    ...
    if batch_operations:
        try:
            ...
            batch_results = await optimized_client.batch_index_files(batch_operations, collection)

            # Create result objects
            for i, (op, success) in enumerate(zip(batch_operations, batch_results, strict=False)):
            ...
            console.print(
                f"[yellow]Batch operation failed, falling back to individual processing: {e}[/yellow]"
            )
...
async def sync_files_from_stdin(
    ...
    try:
        ...
        return await sync_files_from_stdin_async(
            collection, dry_run, verbose, use_stdin, batch_size, parallel, max_workers
        )
...
async def _sync_files_from_stdin_async(
    run_id,
    collection,
    dry_run,
    verbose,
    ...
    try:
        if parallel:
            ...
            async for result_batch in performance_optimizer.optimize_sync_operation(
                changes_generator,
                lambda change: process_change_batch(
...
async def process_change_batch(
    changes: list[dict[str, Any]],
    weaviate: WeaviateService,
    embedding: EmbeddingService,
    collection: str,
    ...
    for delete_op in delete_operations:
        result = await process_single_change(delete_op, weaviate, embedding, collection, dry_run)
